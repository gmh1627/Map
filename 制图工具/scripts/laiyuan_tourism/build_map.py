...
POIS = (
    ...
    PoiSpec("白求恩战地手术室旧址", 114.836, 39.365, "", "", "", 411, 416, 442, 409, 89, "right"),
    ...
    PoiSpec("涞源抗战纪念馆", 114.825, 39.270, "", "", "", 269, 522, 379, 535, 89, "right"),
...
def add_poi_callout(
    ...
    def add_leader(content_right: float) -> None:
        ...
        if poi.name == "白求恩战地手术室旧址":
            leader = add_polyline(
                layout,
                [anchor, QPointF(edge_x, anchor.y())],
                "#3D463E",
                0.27,
                dashed=True,
            )
            leader.setId(f"引线_{poi.name}")
            return
        if abs(anchor.y() - card_center_y) <= mm_y(3.0):
            leader = add_polyline(
                layout,
